...
- Each provider implements a cheap, local-only credential probe (`hasLocalCredentials()`) — the same files, databases, and keychain entries its normal refresh reads, never the network. Providers using the strict credential-boundary contract count only a source proven missing as absent; if credential material exists but is unreadable or malformed, the probe conservatively turns that provider on so its normal refresh can explain the problem instead of silently mistaking it for "not installed" during this one-time check.
