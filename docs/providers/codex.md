...
- Extra accounts are found automatically at launch, in both places the Codex CLI can keep a login: any sibling `~/.codex*` folder holding an `auth.json`, and the CLI's keychain entries (its keyring mode keeps one keychain item per Codex home). A folder and its keychain item are recognized as the same account. A keychain-only login counts only once it's in ongoing use (rotating past its first day) — leftover one-shot entries are ignored; a folder with an `auth.json` counts right away. The first refresh of a keychain-backed extra account can show macOS's usual one-time keychain permission dialog, once per account.
