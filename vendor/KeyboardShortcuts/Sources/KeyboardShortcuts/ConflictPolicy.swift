...
extension EnvironmentValues {
	// Vendored patch: replaced SwiftUI's `@Entry` macro with the manual EnvironmentKey it expands to, so
	// this package builds under the Command Line Tools toolchain (which ships no SwiftUI macro plugin).
	...
	private struct ConflictPolicyKey: EnvironmentKey {
		static var defaultValue: KeyboardShortcuts.ConflictPolicy { .default }
	}
}
