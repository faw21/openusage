...
- **Structure:** one folder per provider with an auth store (reads credentials already on the user's machine), a usage client (calls the provider API), and a mapper (normalizes to `MetricLine`), conforming to `ProviderRuntime` — `refresh()` plus `hasLocalCredentials()`, the local-only credential probe used by first-run detection (`FirstRunSeeder`) and by new-provider detection on the first launch after the provider ships (`NewProviderSeeder`). Use the same auth-store load for both paths: reserve `nil` / an empty result for credentials that are proven absent, and throw after logging when a present file, SQLite database, or keychain entry is unreadable or malformed. Because provider detection is one-shot, `hasLocalCredentials()` must conservatively return `true` for that thrown/indeterminate case so `refresh()` can surface the friendly repair error instead of silently leaving the provider off. See `docs/adding-a-provider.md` and `docs/provider-enablement.md`.
