...
final class AppContainer {
    ...
    init(isFreshInstall: Bool = false) {
        ...
        let balances = BalanceStore(sources: [
            OpenRouterBalanceSource(),
            OpenAISpendSource(),
            AnthropicSpendSource(),
            PerplexitySource(),
            WebshareSource(),
            GCPBillingSource()
        ])
    ...
    /// device identity, provider credentials, and cached usage snapshots. (This fork ships no
    /// telemetry, so upstream's analytics carve-out doesn't apply.)
